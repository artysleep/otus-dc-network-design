...
| eth1.500 | 172.16.100.1/30 | Линк для маршрутизации c VRF-A| 
| eth1.501 | 172.16.200.1/30 | Линк для маршрутизации c VRF-B | 

VLAN 10 и VLAN 30 - это зона безопасности VRF-A, VLAN 20 и VLAN 40 - это зона безопасности VRF-B:
...
!
vlan 500
   name FIREWALL-ZONE-A
!
vlan 501
   name FIREWALL-ZONE-B
!
vrf instance VRF-A
!
vrf instance VRF-B
...
interface Ethernet8
   ...
   mtu 9214
   switchport trunk allowed vlan 10,20,30,40,500-501
...
interface Vlan20
   description VLAN20-VRF-B
   vrf VRF-B
   ip address 192.168.20.104/24
   ip virtual-router address 192.168.20.254/24
!
interface Vlan30
   description VLAN30-VRF-A
   vrf VRF-A
   ip address 192.168.30.104/24
   ip virtual-router address 192.168.30.254/24
!
interface Vlan40
   description VLAN30-VRF-B
   vrf VRF-B
   ip address 192.168.40.104/24
   ip virtual-router address 192.168.40.254/24
!
interface Vlan500
   vrf VRF-A
...
router bgp 65004
   ...
   vrf VRF-B
      ...
      timers bgp 3 9
      neighbor 172.16.200.1 remote-as 65500
      !
      address-family ipv4
